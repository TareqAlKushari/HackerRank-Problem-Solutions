...
There is a shop with old-fashioned cash registers where prices are entered manually, leading to potential errors. 
Given a list of items and their correct prices, compare them to the prices entered a time of sale and determine the number of errors.
...
def priceCheck(products, productPrices, productSold, soldPrice):
    # Step 1: Create a map from product name to correct price
    price_map = {product: price for product, price in zip(products, productPrices)}
    
    # Step 2: Count the number of mismatched prices
    error_count = 0
    for sold_item, sold_price in zip(productSold, soldPrice):
        if price_map[sold_item] != sold_price:
            error_count += 1
...
if __name__ == "__main__":
    ...
    print(priceCheck(products, productPrices, productSold, soldPrice))  # Output: 2